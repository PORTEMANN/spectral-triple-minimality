...
\begin{definition}[Finite spectral triple \cite{krajewski1998}]
A finite spectral triple $(\mathcal{A}_F, \mathcal{H}_F, D_F)$ consists of:
\begin{enumerate}
    \item A finite-dimensional real $C^*$-algebra $\mathcal{A}_F = \bigoplus_{i=1}^{k} M_{n_i}(\mathbb{K}_i)$ with $\mathbb{K}_i \in \{\mathbb{R}, \mathbb{C}, \mathbb{H}\}$.
...
The enumeration was performed by the Python module \texttt{enumeration.py} (see repository \cite{portemann2027repo}). Protocol C12.1 (freeze ID \texttt{C12.1-krajewski}) was applied: enumeration parameters were fixed before execution, and each solution was hashed with SHA-256. The total count 63,160 was verified by an independent run on a different machine. The checksum is registered as \texttt{7818d566...750ec}.
\end{proof}
